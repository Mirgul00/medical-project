...
except ImportError:  # Allows local `cd backend && uvicorn main:app`.
    from database import engine, SessionLocal, Base
    from models import (
        AdminUser,
        Appointment,
...
app = FastAPI(title="Dadanova API")

UPLOAD_DIR = Path(os.getenv("UPLOAD_DIR", PROJECT_ROOT / "frontend" / "assets" / "img" / "uploads"))
...
def ensure_schema():
    ...
    try:
        ...
        logger.exception("Could not create appointment slot uniqueness index")
...
async def save_uploaded_image(file: UploadFile):
    ...
    if PUBLIC_API_URL:
        return f"{PUBLIC_API_URL}/uploads/{filename}"
...
def create_appointment(
    ...
    start_minutes = parse_time_minutes(time)
    end_minutes = start_minutes + parse_duration_minutes(service.duration if service else "")
    overlapping = db.query(Appointment).filter(
        Appointment.date == date,
        Appointment.status != "cancelled",
    ).all()

    if any(appointment_overlaps(db, item, start_minutes, end_minutes) for item in overlapping):
        return {
            "success": False,
